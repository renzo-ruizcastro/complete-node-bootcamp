...
exports.getAllTours = async (req, res) => {
  try {
    ...
    let query = new APIQuery(Tour.find(), req.query);
    if (query.validate())
      query = query.find().sort().select().limit().paginate();
    else query = query.find().sort('-createdAt').select('-__v');

    // EXECUTING QUERY
    // using exec for promises: https://mongoosejs.com/docs/promises.html
    const tours = await query.query.exec();
